...
export const PdfViewer: React.FC<PdfViewerProps> = ({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      if (!sourceId || !pdfPages.length || !highlightText.trim()) return;
      ...
      try {
        const result = await getPdfExcerpt(sourceId, pdfPages, highlightText);
        ...
        if (!cancelled) {
          setError(err?.message || "Không thể tải trích đoạn PDF.");
        }
      } finally {
        if (!cancelled) {
          setIsLoading(false);
    ...
    <div className="flex-1 min-h-0 flex flex-col border border-gray-200 rounded-xl overflow-hidden bg-gray-100">
      <div className="shrink-0 px-3 py-2 bg-white border-b border-gray-200 flex items-center justify-between">
        ...
        <span className="text-[11px] text-gray-500">
          {highlightMode === "exact" && "Đã xác định đoạn trích"}
          {highlightMode === "partial" && "Đã xác định một phần đoạn trích"}
          {highlightMode === "none" && "Chưa xác định vị trí tô sáng"}
        </span>
      </div>
